...
Dont have (*DONT_TOUCH=\"true\"*) before wire 

delete layer0, save layer1~4, 
layer1: 8->3->4->5
last layer 32 
new structure 14*14 input
'''

import numpy as np
import os
import shutil
# from test_unit_layer3_2 import dump_unit_layer1
from test_unit_layer4_3 import dump_unit_layer2
from test_unit_layer4_3 import dump_unit_layer3
from test_unit_layer4_3 import dump_unit_layer4
...
# python test_unit_layer_all4_3.py
def dump_unit_layer1(filename, layer_idx, input_fm, weights, pre_pad_stride, pad_stride, next_pad_stride, max_stride):
        ...
        for j in range(input_fm_c):   # 4
            for k in range(input_fm.shape[1]):  # 16
                for l in range(input_fm.shape[2]):  # 16            
                    ...
                    # f.write("(*DONT_TOUCH=\"true\"*) wire " + str(P_unit_idx) + ";\n")
            ...
            for i in range (out_fm_c):  # 8
                for j in range(conv_fm_h):  # 7
                    for k in range(conv_fm_w):  # 7
                        ...
                        h += 1
        # f.write("endmodule")
        f.write("//layer1 done, begain next layer\n")
        print("layer1 done, next layer ctrl F layer0")
    f.close()
...
if __name__ == "__main__":
    for i in range(5):
        ...
        #     file_p = "test_unit_layer_all.v"
        #     pre_pad_stride_p = 1
        #     pad_stride_p =  1
        #     next_pad_stride_p = 1
        #     max_stride_p = 2
        #     filename = set_dir(filepath = 'test_unit', file = str(file_p))
        #     input_fm = np.load(str(input_fm_p))
        #     weights = np.load(str(weights_p))
            
        #     dump_unit_layer0(filename, layer_idx, input_fm, weights, \
        ...
        if i == 0:
            input_fm_p = "../extract/extract_out/fm_14_5/feature1.fm.npy"
            weights_p = "../extract/extract_out/param_14_5/features1.0.weight.npy"

            file_p = "test_unit_all_layer_14_5.v"
            ...
            input_fm = np.load(str(input_fm_p))
            weights = np.load(str(weights_p))
            # filename = "test_unit/test_unit_layer_all.v"
            dump_unit_layer1(filename, layer_idx, input_fm, weights, \
                    pre_pad_stride = pre_pad_stride_p, \
                    pad_stride = pad_stride_p, \
                    next_pad_stride = next_pad_stride_p, \
                    max_stride = max_stride_p)
        ...
        if i == 1:
            input_fm_p = "../extract/extract_out/fm_14_5/feature2.fm.npy"
            weights_p = "../extract/extract_out/param_14_5/features2.0.weight.npy"
            # file_p = "test_unit_all_layer_14.v"
            pre_pad_stride_p = 0
            pad_stride_p =  0
            next_pad_stride_p = 0
            max_stride_p = 1
            input_fm = np.load(str(input_fm_p))
            weights = np.load(str(weights_p))
            filename = "test_unit/test_unit_all_layer_14_5.v"
            dump_unit_layer2(filename, layer_idx, input_fm, weights, \
                    pre_pad_stride = pre_pad_stride_p, \
                    pad_stride = pad_stride_p, \
                    next_pad_stride = next_pad_stride_p, \
                    max_stride = max_stride_p)
        ...
        if i == 2:
            input_fm_p = "../extract/extract_out/fm_14_5/feature3.fm.npy"
            weights_p = "../extract/extract_out/param_14_5/features3.0.weight.npy"
            ...
            weights = np.load(str(weights_p))
            filename = "test_unit/test_unit_all_layer_14_5.v"
            dump_unit_layer3(filename, layer_idx, input_fm, weights, \
                    pre_pad_stride = pre_pad_stride_p, \
                    pad_stride = pad_stride_p, \
                    next_pad_stride = next_pad_stride_p, \
                    max_stride = max_stride_p)
        ...
        if i == 3:
            input_fm_p = "../extract/extract_out/fm_14_5/feature4.fm.npy"
            weights_p = "../extract/extract_out/param_14_5/features4.0.weight.npy"
            ...
            weights = np.load(str(weights_p))
            filename = "test_unit/test_unit_all_layer_14_5.v"
            dump_unit_layer4(filename, layer_idx, input_fm, weights, \
                    pre_pad_stride = pre_pad_stride_p, \
                    pad_stride = pad_stride_p, \
                    next_pad_stride = next_pad_stride_p, \
                    max_stride = max_stride_p)
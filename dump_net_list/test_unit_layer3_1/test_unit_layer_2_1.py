...
def dump_unit_layer1(filename, layer_idx, input_fm, weights, pre_pad_stride, pad_stride, next_pad_stride, max_stride):
                ...
                if k <= 3:
                    for l in range(input_fm.shape[2]):  # 16 
                        if l <= 3:           
                            ...
                            if k == 0 or l == 0 or k == (input_fm.shape[1] - 1) or l == (input_fm.shape[2] - 1):
            ...
            for m in range(input_fm.shape[2]):  # 16
                if l <= 3:
                    if m <= 3:
                        ...
                        if m == 0 or m == (input_fm.shape[2] - 1) or l == 0 or l == (input_fm.shape[1] - 1) :
                            if value_temp == -1:
                                value_temp = 0
    ...
    with open (filename, 'a') as f:
        ...
        print("layer1 done, next layer ctrl F layer1")
    f.close()
